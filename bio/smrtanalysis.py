...
def blasr(infile, infile_long, outfile, outfile_fofn, sam=False):
...
filterm4.py {outfile} > {outfile_filtered} 2> {outfile_filtered}.log'""".format(
        ...
        threads=config.param('smrtanalysis_blasr', 'num_threads', type='posint'),
...
def filtering(fofn, input_xml, params_xml, output_dir, log):
    ...
    return Job(
        ...
        [input_fofn, output_fastq, output_prefix + "fasta", os.path.join(output_dir, "data", "filtered_summary.csv"), 
         os.path.join(output_dir, "data", "filtered_subreads.fasta")
        ],
        [
...
getNumberOfBasesInFasta.pl --infile {output_dir}/data/filtered_subreads.fasta > {output_dir}/data/number_of_bp.txt""".format(
        ...
        threads=config.param('smrtanalysis_filtering', 'num_threads'),
...
pbalign \\
  ...
  --regionTable={control_regions_fofn}'""".format(
        ...
        cmph5=cmph5,
        tmp_dir=tmp_dir,
        threads=config.param('smrtanalysis_pbalign', 'num_threads', type='posint'),
...
awk '{{if (\$0~/>/) {{sub(/>/,\\"@\\",\$0);print;}} else {{l=length(\$0);q=\\"\\"; while (l--) {{q=q \\"9\\"}}printf(\\"%s\\n+\\n%s\\n\\",\$0,q)}}}}' {outfile} \\
  > {outfile_fastq}""".format(
        threads=config.param('smrtanalysis_pbdagcon', 'num_threads', type='posint'),
        infile=infile,
        outfile=outfile,
        outfile_fastq=outfile_fastq
    ))

def pbutgcns(gpk_store, tig_store, unitigs_list, prefix, outdir, outfile, tmp_dir, assembly_done):
...
pbutgcns_wf.sh'""".format(
        ...
        tmp_dir=tmp_dir,
        prefix=prefix,
        threads=config.param('smrtanalysis_pbutgcns', 'num_threads', type='posint'),
        outfile=outfile
    ))
...
def run_ca(infile, ini, prefix, outdir, done_file):
...
gunzip -c \\
  ...
  > {outfile_fasta_uncompressed}""".format(
        ...
        threads=config.param('smrtanalysis_variant_caller', 'num_threads', type='posint'),
