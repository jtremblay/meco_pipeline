...
from bio import nanopore_tools
...
class NanoporeAssembly(common.MECOPipeline):
    ...
    Sample    NanoporeLibrary  EstimatedGenomesize   FASTQ
    LB501T    1                5000000               ./raw_reads/m170517_223644_421.fastq
    LB501T    2                5000000               ./raw_reads/m170517_223644_422.fastq
    LB501T    3                5000000               ./raw_reads/m170517_223644_423.fastq
    """

    def __init__(self):
        self.argparser.add_argument("-r", "--readsets", help="readset file", type=file)
        super(NanoporeAssembly, self).__init__()
    ...
    def filtering(self):
        """
        Filter reads using porechop and then trimmomatic.
        ...
        #outdir = os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])))
        outdir = os.path.relpath(os.getcwd())
        ...
        for sample in self.samples:
            for readset in sample.readsets:
                ...
                mkdir_p(os.path.join(outdir, sample.name, "qced_reads"))
                
                job = nanopore_tools.porechop(
                    readset.fastq_file,
                    #os.path.join(outdir, "qced_reads", readset.sample.name, readset.name),
                    os.path.join(sample.name, "qced_reads", readset.name + "_porechop.fastq.gz")
                )
                job.name = "porechop_" + sample.name + "_" + readset.name
                job.subname = "porechop"
                jobs.append(job)

                job = nanopore_tools.trimmomatic_se(
                    os.path.join(sample.name, "qced_reads",  readset.name + "_porechop.fastq.gz"),
                    os.path.join(sample.name, "qced_reads",  readset.name + "_porechop_trimmed.fastq.gz"),
                    33,                                      
                    os.path.join(sample.name, "qced_reads",  readset.name + "_porechop_trimmed.out"),
                    os.path.join(sample.name, "qced_reads",  readset.name + "_porechop_trimmed.stats.csv")
                )                                                                                     
                job.name = "trimmomatic_" + sample.name + "_" + readset.name 
                job.subname = "trim"
                jobs.append(job)

                qced_fastqs.append(os.path.join(sample.name, "qced_reads", readset.name + "_porechop_trimmed.fastq.gz"))

        job = nanopore_tools.cat_fastqs_and_convert_to_fasta(
            qced_fastqs,
            os.path.join(sample.name, "qced_reads", "filtered_reads.fasta"),
            os.path.join(sample.name, "qced_reads", "number_of_bp.txt")
        )
        job.name = "cat_fastqs_and_convert_to_fasta_" + sample.name 
        job.subname = "cat_fastqs"
        jobs.append(job)
            ...
            for coverage_cutoff in config.param('DEFAULT', 'coverage_cutoff', type='list'):
                ...
                jobs.append(concat_jobs([
                    ...
                    pacbio_tools.get_cutoff(
                        ...
                        os.path.join(sample.name, "qced_reads", "filtered_reads.fasta"),
                        os.path.join(sample.name, "qced_reads", "number_of_bp.txt"),#estimated_coverage,
                        estimated_genome_size,
                        coverage_cutoff,
                        os.path.join(coverage_directory, "preassemblyMinReadSize.txt")
                    )
                ], name="get_cutoff." + sample.name + ".coverage_cutoff" + cutoff_x, subname="get_cutoff"))
